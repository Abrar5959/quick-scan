...
var NodesJs = (function (parameters) {
    ...
    t_NodesJs.nodes.forEach(function (_node2, _node2_i) {
        ...
        var alpha = 0.3 - ((0.3 * d) / 200); 

        // New function to calculate alpha based on distance to edge
        var calculateEdgeAlpha = function(node) {
            var edgeDistances = [
                node[0], 
                node[1], 
                cw - node[0], 
            ...
            var minDistance = Math.min.apply(null, edgeDistances);
            var edgeAlpha = Math.max((minDistance - 20) / 30, 0); 

            return edgeAlpha;
        };

        // Apply the edge alpha calculation
        alpha *= calculateEdgeAlpha(_node);
        alpha *= calculateEdgeAlpha(_node2);
        ...
        ctx.strokeStyle = 'rgba('+t_NodesJs.lineColor.join(',')+','+alpha+')'; // Ensure the color is set correctly
