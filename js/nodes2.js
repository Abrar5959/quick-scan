...
var NodesJs = (function (parameters) {
    ...
    (window.addEventListener ? 'load': 'onload', function () {
        ...
        var step = function () {
            ...
            t_NodesJs.nodes.forEach(function (_node, _node_i) {
                // ... (rest of the node processing code) ...
                ...
                t_NodesJs.nodes.forEach(function (_node2, _node2_i) {
                    ...
                    // Calculate distance to the nearest edge for both nodes
                    var edgeDistance1 = Math.min(_node[0], cw - _node[0], _node[1], ch - _node[1]);
                    var edgeDistance2 = Math.min(_node2[0], cw - _node2[0], _node2[1], ch - _node2[1]);
                    
                    // Adjust alpha based on the distance to the edge
                    var fadeFactor = 100; // Adjust this value to control the fade distance
                    alpha *= Math.min(edgeDistance1, edgeDistance2) / fadeFactor;
                    alpha = Math.max(alpha, 0); // Ensure alpha doesn't go negative
